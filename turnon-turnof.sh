...
status=$(aws ec2 describe-instances --instance-ids EC2-INSTANCE-ID --query 'Reservations[0].Instances[0].State.Name' --output text)
...
case $status in
    ...
    stopped)
        ...
        aws ec2 wait instance-running --instance-ids EC2-INSTANCE-ID

        echo "The instance is now running."

        # return the IP address of the instance

        aws ec2 describe-instances --instance-ids EC2-INSTANCE-ID --query 'Reservations[0].Instances[0].PublicIpAddress' --output text
